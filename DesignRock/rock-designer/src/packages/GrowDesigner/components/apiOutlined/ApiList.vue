          ...
          >
            ...
            <div class="min-w-0 flex-1">
              ...
              <p class="mt-0.5 truncate text-xs text-text-secondary">
                {{ element.description || element.url || '暂无描述' }}
              </p>
            </div>
...
import type { DesignerApiOutlinedItem } from '../types'

defineOptions({ name: 'ApiOutlinedList' })

defineProps<{
  list: DesignerApiOutlinedItem[]
  activeId?: string
}>()

const emit = defineEmits<{
  'update:list': [list: DesignerApiOutlinedItem[]]
  edit: [item: DesignerApiOutlinedItem]
  remove: [id: string]
}>()
</script>
