import { ComputedRef, Ref, WritableComputedRef, computed, ref, unref } from "vue";
import {
  SIDE_BAR_MINI_WIDTH,
...
import { Bean, Autowired } from "@grow-admin-rock/ioc";
import type { MenuSetting, Setting } from "@grow-admin-rock/types";
import TYPES from "#/../beankeys";
import { RootSetting } from "#/RootSetting";
import {addResizeListener, debounceAwait, removeResizeListener} from "@grow-admin-rock/utils";
...
export class MenuSettingData {
  ...
  getTrigger: WritableComputedRef<TriggerEnum>;
  ...
  constructor(
    ...
    const getCollapsed = computed(() => {
        return menuSettingStore.collapsed;
    });
    // Menu Type MenuTypeEnum
    ...
    const getTrigger = computed({
      get:() => menuSettingStore.trigger,
      set: (value) => menuSettingStore.trigger = value
    });
