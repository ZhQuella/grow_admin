<template>
  ...
  <EdgeLabelRenderer>
    <div
      class="process-edge-label nodrag nopan"
    ...
    >
      <span class="process-edge-label__text">{{ data?.label || '连线' }}</span>
      <GrowButton
        text
        size="small"
        class="process-edge-label__delete"
        title="删除连线"
        @click.stop="onRemove"
        @mousedown.stop
      >
    ...
    <div
      class="process-edge-endpoint process-edge-endpoint--source nodrag nopan"
      :class="{ 'is-active': selected || data?.active }"
      :style="{
    ...
    />
    <div
      class="process-edge-endpoint process-edge-endpoint--target nodrag nopan"
      :class="{ 'is-active': selected || data?.active }"
      :style="{
...
.process-edge-label.is-active {
  border-color: var(--primary-color);
  box-shadow: 0 0 0 1px color-mix(in srgb, var(--primary-color) 25%, transparent);
}

.process-edge-label__text {
  padding-right: 2px;
}

.process-edge-label__delete {
...
.process-edge-label:hover .process-edge-label__delete,
.process-edge-label.is-active .process-edge-label__delete {
  display: inline-flex !important;
}

.process-edge-label__delete:hover {
  background: color-mix(in srgb, var(--error-color, #dc2626) 45%, transparent) !important;
  color: #fff !important;
}

.process-edge-label__delete :deep(.grow-iconify) {
  display: inline-flex !important;
  align-items: center;
  justify-content: center;
  width: 12px;
...
.process-edge-label__delete :deep(.grow-iconify svg) {
  display: block;
  width: 12px;
  height: 12px;
}

.process-edge-endpoint {
  position: absolute;
  z-index: 2;
...
.process-edge-endpoint:active {
  cursor: grabbing;
}

.process-edge-endpoint:hover,
.process-edge-endpoint.is-active {
  border-color: var(--primary-color);
  background: var(--component-background-color);
