...
import { TreeNode } from '@grow-admin-rock/types';
        ...
        if (isEmpty(componentNode.val)) {
          ...
          if (fs.existsSync(componentPath)) {
            ...
            if (fs.statSync(componentPath).isDirectory()) {
              ...
              if (components.length === 1) {
                ...
                if (fileTsOrJs) {
                  ...
                  throw new Error(
                    `MonoRepoResolverPlugin can not resolve Module <${updatedId}> at: ${importerId} \n Find ${components.length === 0 ? 'none' : 'multiple'} files at: ${componentPath}/index.(ts|js), please check it. \n All Components: ${components}`
                  );
                }
              }
            } else {
            ...
            if (components.length === 1) {
              ...
              console.warn(
                `MonoRepoResolverPlugin resolve Module <${updatedId}> at: ${importerId}, \n Find multi files, we select the *.(ts|js) \n components: ${JSON.stringify(components)}`
              )
              const fileTsOrJs = find(components, (c) => c.endsWith('.ts') || c.endsWith('.js'))
              if (fileTsOrJs) {
                realPath = fileTsOrJs
            ...
            if (!realPath) {
              throw new Error(
                `MonoRepoResolverPlugin can not resolve Module <${updatedId}> at: ${importerId} \n Find none files at: ${componentPath}/index.(ts|js), please check it. \n All components: ${components}`
              );
            }
          }
