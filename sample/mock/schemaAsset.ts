...
type DatabaseSchema = Recordable<any>

type SchemaAssetVersion = {
  version: string
...
function toListItem(asset: SchemaAsset) {
...
function nextVersionLabel(asset: SchemaAsset): string {
...
const assetStore = new Map<string, SchemaAsset>()

function seed() {
  if (assetStore.size) return
  const t1 = '2026-08-01T08:00:00.000Z'
  const t2 = '2026-08-10T10:30:00.000Z'
  const published: SchemaAsset = {
    id: 'sa_demo_1',
    name: '示例销售库',
    code: 'demo_sales_schema',
    description: '演示已发布数据库模型',
    enabled: true,
    currentVersion: 'v1',
    publishedAt: t2,
  ...
  const draft: SchemaAsset = {
    id: 'sa_demo_2',
    name: '示例草稿库',
    code: 'demo_draft_schema',
    description: '演示草稿数据库模型',
    enabled: true,
    currentVersion: null,
    publishedAt: null,
    publishedBy: null,
  ...
  const disabled: SchemaAsset = {
    id: 'sa_demo_3',
    name: '已停用模型',
    code: 'demo_disabled_schema',
    description: '停用后可删除',
    enabled: false,
    currentVersion: 'v2',
...
function filterAssets(query: Recordable<any>): SchemaAsset[] {
...
const mocks: MockMethod[] = [
  {
    url: mockUrl('/schema-assets/page'),
    method: 'post',
    timeout: 80,
    response: ({ body }) => {
  ...
  {
    url: mockUrl('/schema-asset/detail'),
    method: 'post',
    timeout: 80,
    response: ({ body }) => {
  ...
  {
    url: mockUrl('/schema-asset/delete'),
    method: 'post',
    timeout: 80,
    response: ({ body }) => {
      const id = String((body as Recordable<any>)?.id || '')
  ...
  {
    url: mockUrl('/schema-asset/enabled'),
    method: 'put',
    timeout: 80,
    response: ({ body }) => {
      const payload = (body || {}) as Recordable<any>
  ...
  {
    url: mockUrl('/schema-asset/schema'),
    method: 'put',
    timeout: 120,
    response: ({ body }) => {
      const payload = (body || {}) as Recordable<any>
  ...
  {
    url: mockUrl('/schema-asset/publish'),
    method: 'post',
    timeout: 120,
    response: ({ body }) => {
      const payload = (body || {}) as Recordable<any>
  ...
  {
    url: mockUrl('/schema-asset/versions'),
    method: 'post',
    timeout: 80,
    response: ({ body }) => {
      const id = String((body as Recordable<any>)?.id || '')
  ...
  {
    url: mockUrl('/schema-asset/rollback'),
    method: 'post',
    timeout: 60,
