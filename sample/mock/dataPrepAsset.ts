...
type DataPrepDataset = Recordable<any>

type DataPrepAssetVersion = {
  version: string
...
function emptySchema(): DataPrepDataset {
  return {
    version: 1,
    id: `dataset_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`,
    name: '未命名数据集',
    description: '',
    schemaRefs: [],
    sources: [],
...
function nextId() {
  return `dpa_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`
}

function toListItem(asset: DataPrepAsset) {
...
function nextVersionLabel(asset: DataPrepAsset): string {
  const max = asset.versions.reduce((acc, item) => Math.max(acc, parseVersionNo(item.version)), 0)
  return `v${max + 1}`
}

function cloneSchema(schema?: DataPrepDataset): DataPrepDataset {
  try {
...
const assetStore = new Map<string, DataPrepAsset>()

function seed() {
  if (assetStore.size) return
  const t1 = '2026-08-01T08:00:00.000Z'
  const t2 = '2026-08-10T10:30:00.000Z'
  const published: DataPrepAsset = {
    id: 'dpa_demo_1',
    name: '示例销售汇总',
    code: 'demo_sales_dataset',
    description: '演示已发布数据准备资产',
    enabled: true,
    currentVersion: 'v1',
    publishedAt: t2,
  ...
  const draft: DataPrepAsset = {
    id: 'dpa_demo_2',
    name: '示例草稿数据集',
    code: 'demo_draft_dataset',
    description: '演示草稿数据准备资产',
    enabled: true,
    currentVersion: null,
    publishedAt: null,
    publishedBy: null,
  ...
  const disabled: DataPrepAsset = {
    id: 'dpa_demo_3',
    name: '已停用数据集',
    code: 'demo_disabled_dataset',
    description: '停用后可删除',
    enabled: false,
    currentVersion: 'v2',
...
function filterAssets(query: Recordable<any>): DataPrepAsset[] {
...
const mocks: MockMethod[] = [
  {
    url: mockUrl('/data-prep-assets/page'),
    method: 'post',
    timeout: 80,
    response: ({ body }) => {
  ...
  {
    url: mockUrl('/data-prep-asset/detail'),
    method: 'post',
    timeout: 80,
    response: ({ body }) => {
  ...
  {
    url: mockUrl('/data-prep-asset/delete'),
    method: 'post',
    timeout: 80,
    response: ({ body }) => {
      const id = String((body as Recordable<any>)?.id || '')
  ...
  {
    url: mockUrl('/data-prep-asset/enabled'),
    method: 'put',
    timeout: 80,
    response: ({ body }) => {
      const payload = (body || {}) as Recordable<any>
  ...
  {
    url: mockUrl('/data-prep-asset/schema'),
    method: 'put',
    timeout: 120,
    response: ({ body }) => {
      const payload = (body || {}) as Recordable<any>
  ...
  {
    url: mockUrl('/data-prep-asset/publish'),
    method: 'post',
    timeout: 120,
    response: ({ body }) => {
      const payload = (body || {}) as Recordable<any>
  ...
  {
    url: mockUrl('/data-prep-asset/versions'),
    method: 'post',
    timeout: 80,
    response: ({ body }) => {
      const id = String((body as Recordable<any>)?.id || '')
  ...
  {
    url: mockUrl('/data-prep-asset/rollback'),
    method: 'post',
    timeout: 60,
