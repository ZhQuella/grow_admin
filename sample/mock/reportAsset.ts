...
type ReportSchema = Recordable<any>

type ReportAssetVersion = {
  version: string
  schema: ReportSchema
  publishedAt: string
  publishedBy?: string
...
function emptySchema(): ReportSchema {
  return {
    layout: [],
    pageConfig: {},
    dataSource: [],
...
function toListItem(asset: ReportAsset) {
...
function nextVersionLabel(asset: ReportAsset): string {
...
const assetStore = new Map<string, ReportAsset>()

function seed() {
  if (assetStore.size) return
  const t1 = '2026-08-01T08:00:00.000Z'
  const t2 = '2026-08-10T10:30:00.000Z'
  const published: ReportAsset = {
    id: 'ra_demo_1',
    name: '示例经营看板',
    code: 'demo_biz_dashboard',
    description: '演示已发布报表资产',
    enabled: true,
    currentVersion: 'v1',
    publishedAt: t2,
  ...
  const draft: ReportAsset = {
    id: 'ra_demo_2',
    name: '示例销售报表',
    code: 'demo_sales_report',
    description: '演示草稿报表资产',
    enabled: true,
    currentVersion: null,
    publishedAt: null,
    publishedBy: null,
  ...
  const disabled: ReportAsset = {
    id: 'ra_demo_3',
    name: '已停用报表',
    code: 'demo_disabled_report',
    description: '停用后可删除',
    enabled: false,
    currentVersion: 'v2',
...
function filterAssets(query: Recordable<any>): ReportAsset[] {
...
const mocks: MockMethod[] = [
  {
    url: mockUrl('/report-assets/page'),
    method: 'post',
    timeout: 80,
    response: ({ body }) => {
  ...
  {
    url: mockUrl('/report-asset/detail'),
    method: 'post',
    timeout: 80,
    response: ({ body }) => {
  ...
  {
    url: mockUrl('/report-asset/delete'),
    method: 'post',
    timeout: 80,
    response: ({ body }) => {
      const id = String((body as Recordable<any>)?.id || '')
  ...
  {
    url: mockUrl('/report-asset/enabled'),
    method: 'put',
    timeout: 80,
    response: ({ body }) => {
      const payload = (body || {}) as Recordable<any>
  ...
  {
    url: mockUrl('/report-asset/schema'),
    method: 'put',
    timeout: 120,
    response: ({ body }) => {
      const payload = (body || {}) as Recordable<any>
  ...
  {
    url: mockUrl('/report-asset/publish'),
    method: 'post',
    timeout: 120,
    response: ({ body }) => {
      const payload = (body || {}) as Recordable<any>
  ...
  {
    url: mockUrl('/report-asset/versions'),
    method: 'post',
    timeout: 80,
    response: ({ body }) => {
      const id = String((body as Recordable<any>)?.id || '')
  ...
  {
    url: mockUrl('/report-asset/rollback'),
    method: 'post',
    timeout: 60,
