...
type ProcessFlow = Recordable<any>

type ProcessAssetVersion = {
  version: string
  schema: ProcessFlow
  publishedAt: string
  publishedBy?: string
...
function emptySchema(): ProcessFlow {
  return {
    version: 1,
    id: `process_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`,
    name: '未命名流程',
    status: 'draft',
    nodes: [],
    edges: [],
...
function toListItem(asset: ProcessAsset) {
...
function nextVersionLabel(asset: ProcessAsset): string {
  const max = asset.versions.reduce((acc, item) => Math.max(acc, parseVersionNo(item.version)), 0)
  return `v${max + 1}`
}

function cloneSchema(schema?: ProcessFlow): ProcessFlow {
  try {
...
const assetStore = new Map<string, ProcessAsset>()

function seed() {
  if (assetStore.size) return
  const t1 = '2026-08-01T08:00:00.000Z'
  const t2 = '2026-08-10T10:30:00.000Z'
  const published: ProcessAsset = {
    id: 'pa_demo_1',
    name: '示例审批流程',
    code: 'demo_approval_flow',
    description: '演示已发布流程资产',
    enabled: true,
    currentVersion: 'v1',
    publishedAt: t2,
  ...
  const draft: ProcessAsset = {
    id: 'pa_demo_2',
    name: '示例草稿流程',
    code: 'demo_draft_process',
    description: '演示草稿流程资产',
    enabled: true,
    currentVersion: null,
    publishedAt: null,
    publishedBy: null,
  ...
  const disabled: ProcessAsset = {
    id: 'pa_demo_3',
    name: '已停用流程',
    code: 'demo_disabled_process',
    description: '停用后可删除',
    enabled: false,
    currentVersion: 'v2',
...
function filterAssets(query: Recordable<any>): ProcessAsset[] {
...
const mocks: MockMethod[] = [
  {
    url: mockUrl('/process-assets/page'),
    method: 'post',
    timeout: 80,
    response: ({ body }) => {
  ...
  {
    url: mockUrl('/process-asset/detail'),
    method: 'post',
    timeout: 80,
    response: ({ body }) => {
  ...
  {
    url: mockUrl('/process-asset/delete'),
    method: 'post',
    timeout: 80,
    response: ({ body }) => {
      const id = String((body as Recordable<any>)?.id || '')
  ...
  {
    url: mockUrl('/process-asset/enabled'),
    method: 'put',
    timeout: 80,
    response: ({ body }) => {
      const payload = (body || {}) as Recordable<any>
  ...
  {
    url: mockUrl('/process-asset/schema'),
    method: 'put',
    timeout: 120,
    response: ({ body }) => {
      const payload = (body || {}) as Recordable<any>
  ...
  {
    url: mockUrl('/process-asset/publish'),
    method: 'post',
    timeout: 120,
    response: ({ body }) => {
      const payload = (body || {}) as Recordable<any>
  ...
  {
    url: mockUrl('/process-asset/versions'),
    method: 'post',
    timeout: 80,
    response: ({ body }) => {
      const id = String((body as Recordable<any>)?.id || '')
  ...
  {
    url: mockUrl('/process-asset/rollback'),
    method: 'post',
    timeout: 60,
