...
import { fetchLowcodeAssetPage } from '../../../../api/lowcodeAsset'
...
export function useManageTable() {
  ...
  const tableData = ref<LowcodeAssetListItem[]>([])
  const total = ref(0)
  const page = ref(1)
  ...
  const tableColumns = ref<ManageTableColumn[]>([
    ...
    { title: '类型', field: 'type', visible: true, minWidth: 90 },
  ...
  async function loadList() {
    ...
    try {
      ...
      const data = await fetchLowcodeAssetPage({
        page: page.value,
