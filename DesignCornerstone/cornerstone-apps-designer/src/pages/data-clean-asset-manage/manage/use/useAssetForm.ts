...
import { createDataCleanAsset, updateDataCleanAsset } from '../../../../api/dataCleanAsset'
...
export function useAssetForm(options: UseAssetFormOptions) {
  ...
  function openEdit(row: DataCleanAssetListItem) {
    formMode.value = 'edit'
    formModel.id = row.id
  ...
  async function submitForm() {
    ...
    try {
      if (formMode.value === 'create') {
        ...
        await updateDataCleanAsset(formModel.id, {
          name: formModel.name.trim(),
          description: formModel.description.trim(),
        })
