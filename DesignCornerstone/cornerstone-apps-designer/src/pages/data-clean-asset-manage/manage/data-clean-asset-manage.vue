<template>
  <div class="data-clean-asset-manage">
    <GrowRow justify="space-between" class="data-clean-asset-manage__toolbar">
      <GrowCol :span="14">
        <GrowSpace>
      ...
      <GrowCol :span="10">
        <div class="data-clean-asset-manage__toolbar-options">
    ...
    <div class="data-clean-asset-manage__table">
      <GrowWatchBox class="data-clean-asset-manage__watch">
            ...
            >
              <template #default="{ row }">
                ...
                <template v-else-if="col.field === 'actions'">
                  <div class="data-clean-asset-manage__actions">
    ...
    <div class="data-clean-asset-manage__pager">
      <GrowPagination
    ...
    <GrowDialog
      v-model="formVisible"
      :title="formMode === 'create' ? '新增数据清洗资产' : '编辑基础信息'"
      width="520px"
      append-to-body
      destroy-on-close
    >
    ...
    <GrowDialog
      ...
      destroy-on-close
    >
      <p class="data-clean-asset-manage__hint">
    ...
    <GrowDrawer
      ...
      destroy-on-close
    >
      <div class="data-clean-asset-manage__version-head">
        <div class="data-clean-asset-manage__version-head-main">
          <span>{{ versionAsset?.name }}</span>
          <span v-if="versionAsset?.code" class="data-clean-asset-manage__muted">
            {{ versionAsset.code }}
          </span>
        </div>
        <span class="data-clean-asset-manage__muted">
          当前版本：{{ versionCurrent || '-' }}
        </span>
      </div>
      <p v-if="versionLoading" class="data-clean-asset-manage__hint">加载中…</p>
      <div v-else-if="!versionRows.length" class="data-clean-asset-manage__version-empty">
        暂无发布版本
      </div>
      <GrowTimeline v-else class="data-clean-asset-manage__version-timeline">
        <GrowTimelineItem
        ...
        >
          <div
            class="data-clean-asset-manage__version-card"
            :class="{ 'is-current': row.isCurrent }"
          >
            <div class="data-clean-asset-manage__version-card-top">
              <div class="data-clean-asset-manage__version-card-title">
                <span class="data-clean-asset-manage__version-no">{{ row.version }}</span>
            ...
            <div class="data-clean-asset-manage__version-meta">
              <div class="data-clean-asset-manage__version-meta-item">
                <span class="data-clean-asset-manage__version-meta-label">发布时间</span>
                <span>{{ formatTime(row.publishedAt) }}</span>
              </div>
              <div class="data-clean-asset-manage__version-meta-item">
                <span class="data-clean-asset-manage__version-meta-label">发布人</span>
                <span>{{ row.publishedBy || '-' }}</span>
              </div>
            </div>
            <div class="data-clean-asset-manage__version-remark-block">
              <span class="data-clean-asset-manage__version-meta-label">发布备注</span>
              <p class="data-clean-asset-manage__version-remark">
                {{ row.remark || '无备注' }}
              </p>
            </div>
          </div>
...
import { useDataCleanAssetManage } from './use/useDataCleanAssetManage'

defineOptions({
  name: 'DataCleanAssetManagePage',
})
...
const {
  ...
  onRollback,
  formatTime,
} = useDataCleanAssetManage()
</script>

<style scoped>
.data-clean-asset-manage {
  display: flex;
...
.data-clean-asset-manage__toolbar {
  display: flex;
  align-items: center;
  padding: 10px 12px;
  border-radius: 8px;
...
.data-clean-asset-manage__toolbar-options {
  display: flex;
  align-items: center;
...
.data-clean-asset-manage__table {
  flex: 1;
  min-height: 0;
  margin-top: 10px;
  overflow: hidden;
...
.data-clean-asset-manage__watch {
  height: 100%;
  min-height: 0;
}

.data-clean-asset-manage__pager {
  display: flex;
  justify-content: flex-end;
  padding: 12px;
  background: var(--component-background-color);
...
.data-clean-asset-manage__hint {
  margin: 0 0 12px;
  color: var(--text-color-secondary);
  line-height: 1.6;
}

.data-clean-asset-manage__version-head {
  display: flex;
...
.data-clean-asset-manage__version-head-main {
  display: flex;
  flex-direction: column;
  gap: 4px;
  min-width: 0;
...
.data-clean-asset-manage__version-empty {
  padding: 40px 0;
  text-align: center;
  color: var(--text-color-secondary);
  font-size: 13px;
...
.data-clean-asset-manage__version-timeline {
  padding-left: 4px;
}

.data-clean-asset-manage__version-card {
  padding: 12px;
  border-radius: 8px;
  background: var(--layout-color);
}

.data-clean-asset-manage__version-card.is-current {
  background: color-mix(in srgb, var(--el-color-primary, #409eff) 8%, transparent);
}

.data-clean-asset-manage__version-card-top {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 8px;
...
.data-clean-asset-manage__version-card-title {
  display: flex;
  align-items: center;
  gap: 8px;
  min-width: 0;
...
.data-clean-asset-manage__version-no {
  font-size: 15px;
  font-weight: 600;
  color: var(--text-color);
}

.data-clean-asset-manage__version-meta {
  display: grid;
...
.data-clean-asset-manage__version-meta-item {
  display: flex;
  flex-direction: column;
  gap: 2px;
  min-width: 0;
...
.data-clean-asset-manage__version-meta-label {
  font-size: 12px;
  color: var(--text-color-secondary);
}

.data-clean-asset-manage__version-remark-block {
  display: flex;
  flex-direction: column;
  gap: 4px;
  margin-top: 10px;
...
.data-clean-asset-manage__version-remark {
  margin: 0;
  font-size: 13px;
  line-height: 1.5;
  color: var(--text-color);
...
.data-clean-asset-manage__muted {
  color: var(--text-color-secondary);
  font-weight: 400;
  font-size: 13px;
}

.data-clean-asset-manage__actions {
  display: inline-flex;
  align-items: center;
  gap: 2px;
  flex-wrap: nowrap;
