...
import { createOnlinePage, updateOnlinePage } from '../../../../api/onlinePage'
...
export function useAssetForm(options: UseAssetFormOptions) {
  ...
  function openEdit(row: OnlinePageListItem) {
    formMode.value = 'edit'
    formModel.id = row.id
  ...
  async function submitForm() {
    ...
    try {
      if (formMode.value === 'create') {
        ...
        await updateOnlinePage(formModel.id, {
          name: formModel.name.trim(),
          description: formModel.description.trim(),
        })
