<template>
  <div class="online-page-manage">
    <GrowRow justify="space-between" class="online-page-manage__toolbar">
      <GrowCol :span="14">
        <GrowSpace>
      ...
      <GrowCol :span="10">
        <div class="online-page-manage__toolbar-options">
    ...
    <div class="online-page-manage__table">
      <GrowWatchBox class="online-page-manage__watch">
            ...
            >
              <template #default="{ row }">
                ...
                <template v-else-if="col.field === 'actions'">
                  <div class="online-page-manage__actions">
                    ...
                    <GrowTooltip content="设计" placement="top">
                      <GrowButton link type="primary" @click="goDesign(row)">
                        <GrowIconify icon="ant-design:code-sandbox-outlined" :size="16" />
                      </GrowButton>
                    </GrowTooltip>
    ...
    <div class="online-page-manage__pager">
      <GrowPagination
    ...
    <GrowDialog
      v-model="formVisible"
      :title="formMode === 'create' ? '新增在线页面' : '编辑基础信息'"
      width="520px"
      append-to-body
      destroy-on-close
    >
    ...
    <GrowDialog
      ...
      destroy-on-close
    >
      <p class="online-page-manage__hint">
    ...
    <GrowDrawer
      ...
      destroy-on-close
    >
      <div class="online-page-manage__version-head">
        <div class="online-page-manage__version-head-main">
          <span>{{ versionAsset?.name }}</span>
          <span v-if="versionAsset?.code" class="online-page-manage__muted">
            {{ versionAsset.code }}
          </span>
        </div>
        <span class="online-page-manage__muted">
          当前版本：{{ versionCurrent || '-' }}
        </span>
      </div>
      <p v-if="versionLoading" class="online-page-manage__hint">加载中…</p>
      <div v-else-if="!versionRows.length" class="online-page-manage__version-empty">
        暂无发布版本
      </div>
      <GrowTimeline v-else class="online-page-manage__version-timeline">
        <GrowTimelineItem
        ...
        >
          <div
            class="online-page-manage__version-card"
            :class="{ 'is-current': row.isCurrent }"
          >
            <div class="online-page-manage__version-card-top">
              <div class="online-page-manage__version-card-title">
                <span class="online-page-manage__version-no">{{ row.version }}</span>
            ...
            <div class="online-page-manage__version-meta">
              <div class="online-page-manage__version-meta-item">
                <span class="online-page-manage__version-meta-label">发布时间</span>
                <span>{{ formatTime(row.publishedAt) }}</span>
              </div>
              <div class="online-page-manage__version-meta-item">
                <span class="online-page-manage__version-meta-label">发布人</span>
                <span>{{ row.publishedBy || '-' }}</span>
              </div>
            </div>
            <div class="online-page-manage__version-remark-block">
              <span class="online-page-manage__version-meta-label">发布备注</span>
              <p class="online-page-manage__version-remark">
                {{ row.remark || '无备注' }}
              </p>
            </div>
          </div>
...
import { useOnlinePageManage } from './use/useOnlinePageManage'

defineOptions({
  name: 'OnlinePageManagePage',
})
...
.online-page-manage__toolbar-options {
  display: flex;
  align-items: center;
...
.online-page-manage__watch {
  height: 100%;
  min-height: 0;
}

.online-page-manage__pager {
  display: flex;
  justify-content: flex-end;
  padding: 12px;
  background: var(--component-background-color);
...
.online-page-manage__hint {
  margin: 0 0 12px;
  color: var(--text-color-secondary);
  line-height: 1.6;
}

.online-page-manage__version-head {
  display: flex;
...
.online-page-manage__version-head-main {
  display: flex;
  flex-direction: column;
  gap: 4px;
  min-width: 0;
...
.online-page-manage__version-empty {
  padding: 40px 0;
  text-align: center;
  color: var(--text-color-secondary);
  font-size: 13px;
...
.online-page-manage__version-timeline {
  padding-left: 4px;
}

.online-page-manage__version-card {
  padding: 12px;
  border-radius: 8px;
  background: var(--layout-color);
}

.online-page-manage__version-card.is-current {
  background: color-mix(in srgb, var(--el-color-primary, #409eff) 8%, transparent);
}

.online-page-manage__version-card-top {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 8px;
...
.online-page-manage__version-card-title {
  display: flex;
  align-items: center;
  gap: 8px;
  min-width: 0;
...
.online-page-manage__version-no {
  font-size: 15px;
  font-weight: 600;
  color: var(--text-color);
}

.online-page-manage__version-meta {
  display: grid;
...
.online-page-manage__version-meta-item {
  display: flex;
  flex-direction: column;
  gap: 2px;
  min-width: 0;
...
.online-page-manage__version-meta-label {
  font-size: 12px;
  color: var(--text-color-secondary);
}

.online-page-manage__version-remark-block {
  display: flex;
  flex-direction: column;
  gap: 4px;
  margin-top: 10px;
...
.online-page-manage__version-remark {
  margin: 0;
  font-size: 13px;
  line-height: 1.5;
  color: var(--text-color);
...
.online-page-manage__muted {
  color: var(--text-color-secondary);
  font-weight: 400;
  font-size: 13px;
}

.online-page-manage__actions {
  display: inline-flex;
  align-items: center;
  gap: 2px;
  flex-wrap: nowrap;
