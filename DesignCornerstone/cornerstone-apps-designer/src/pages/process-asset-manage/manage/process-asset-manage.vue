<template>
  <div class="process-asset-manage">
    <GrowRow justify="space-between" class="process-asset-manage__toolbar">
      <GrowCol :span="14">
        <GrowSpace>
      ...
      <GrowCol :span="10">
        <div class="process-asset-manage__toolbar-options">
    ...
    <div class="process-asset-manage__table">
      <GrowWatchBox class="process-asset-manage__watch">
            ...
            >
              <template #default="{ row }">
                ...
                <template v-else-if="col.field === 'actions'">
                  <div class="process-asset-manage__actions">
    ...
    <div class="process-asset-manage__pager">
      <GrowPagination
    ...
    <GrowDialog
      v-model="formVisible"
      :title="formMode === 'create' ? '新增流程资产' : '编辑基础信息'"
      width="520px"
      append-to-body
      destroy-on-close
    >
    ...
    <GrowDialog
      ...
      destroy-on-close
    >
      <p class="process-asset-manage__hint">
    ...
    <GrowDrawer
      ...
      destroy-on-close
    >
      <div class="process-asset-manage__version-head">
        <div class="process-asset-manage__version-head-main">
          <span>{{ versionAsset?.name }}</span>
          <span v-if="versionAsset?.code" class="process-asset-manage__muted">
            {{ versionAsset.code }}
          </span>
        </div>
        <span class="process-asset-manage__muted">
          当前版本：{{ versionCurrent || '-' }}
        </span>
      </div>
      <p v-if="versionLoading" class="process-asset-manage__hint">加载中…</p>
      <div v-else-if="!versionRows.length" class="process-asset-manage__version-empty">
        暂无发布版本
      </div>
      <GrowTimeline v-else class="process-asset-manage__version-timeline">
        <GrowTimelineItem
        ...
        >
          <div
            class="process-asset-manage__version-card"
            :class="{ 'is-current': row.isCurrent }"
          >
            <div class="process-asset-manage__version-card-top">
              <div class="process-asset-manage__version-card-title">
                <span class="process-asset-manage__version-no">{{ row.version }}</span>
            ...
            <div class="process-asset-manage__version-meta">
              <div class="process-asset-manage__version-meta-item">
                <span class="process-asset-manage__version-meta-label">发布时间</span>
                <span>{{ formatTime(row.publishedAt) }}</span>
              </div>
              <div class="process-asset-manage__version-meta-item">
                <span class="process-asset-manage__version-meta-label">发布人</span>
                <span>{{ row.publishedBy || '-' }}</span>
              </div>
            </div>
            <div class="process-asset-manage__version-remark-block">
              <span class="process-asset-manage__version-meta-label">发布备注</span>
              <p class="process-asset-manage__version-remark">
                {{ row.remark || '无备注' }}
              </p>
            </div>
          </div>
...
import { useProcessAssetManage } from './use/useProcessAssetManage'

defineOptions({
  name: 'ProcessAssetManagePage',
})
...
const {
  ...
  onRollback,
  formatTime,
} = useProcessAssetManage()
</script>

<style scoped>
.process-asset-manage {
  display: flex;
...
.process-asset-manage__toolbar-options {
  display: flex;
  align-items: center;
...
.process-asset-manage__watch {
  height: 100%;
  min-height: 0;
}

.process-asset-manage__pager {
  display: flex;
  justify-content: flex-end;
  padding: 12px;
  background: var(--component-background-color);
...
.process-asset-manage__hint {
  margin: 0 0 12px;
  color: var(--text-color-secondary);
  line-height: 1.6;
}

.process-asset-manage__version-head {
  display: flex;
...
.process-asset-manage__version-head-main {
  display: flex;
  flex-direction: column;
  gap: 4px;
  min-width: 0;
...
.process-asset-manage__version-empty {
  padding: 40px 0;
  text-align: center;
  color: var(--text-color-secondary);
  font-size: 13px;
...
.process-asset-manage__version-timeline {
  padding-left: 4px;
}

.process-asset-manage__version-card {
  padding: 12px;
  border-radius: 8px;
  background: var(--layout-color);
}

.process-asset-manage__version-card.is-current {
  background: color-mix(in srgb, var(--el-color-primary, #409eff) 8%, transparent);
}

.process-asset-manage__version-card-top {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 8px;
...
.process-asset-manage__version-card-title {
  display: flex;
  align-items: center;
  gap: 8px;
  min-width: 0;
...
.process-asset-manage__version-no {
  font-size: 15px;
  font-weight: 600;
  color: var(--text-color);
}

.process-asset-manage__version-meta {
  display: grid;
...
.process-asset-manage__version-meta-item {
  display: flex;
  flex-direction: column;
  gap: 2px;
  min-width: 0;
...
.process-asset-manage__version-meta-label {
  font-size: 12px;
  color: var(--text-color-secondary);
}

.process-asset-manage__version-remark-block {
  display: flex;
  flex-direction: column;
  gap: 4px;
  margin-top: 10px;
...
.process-asset-manage__version-remark {
  margin: 0;
  font-size: 13px;
  line-height: 1.5;
  color: var(--text-color);
...
.process-asset-manage__muted {
  color: var(--text-color-secondary);
  font-weight: 400;
  font-size: 13px;
}

.process-asset-manage__actions {
  display: inline-flex;
  align-items: center;
  gap: 2px;
  flex-wrap: nowrap;
