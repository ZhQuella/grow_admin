...
import { createProcessAsset, updateProcessAsset } from '../../../../api/processAsset'
...
export function useAssetForm(options: UseAssetFormOptions) {
  ...
  function openEdit(row: ProcessAssetListItem) {
    formMode.value = 'edit'
    formModel.id = row.id
  ...
  async function submitForm() {
    ...
    try {
      if (formMode.value === 'create') {
        ...
        await updateProcessAsset(formModel.id, {
          name: formModel.name.trim(),
          description: formModel.description.trim(),
        })
