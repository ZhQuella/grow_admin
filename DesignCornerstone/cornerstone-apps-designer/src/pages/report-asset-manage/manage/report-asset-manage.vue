<template>
  <div class="report-asset-manage">
    <GrowRow justify="space-between" class="report-asset-manage__toolbar">
      <GrowCol :span="14">
        <GrowSpace>
      ...
      <GrowCol :span="10">
        <div class="report-asset-manage__toolbar-options">
    ...
    <div class="report-asset-manage__table">
      <GrowWatchBox class="report-asset-manage__watch">
            ...
            >
              <template #default="{ row }">
                ...
                <template v-else-if="col.field === 'actions'">
                  <div class="report-asset-manage__actions">
                    ...
                    <GrowTooltip content="设计" placement="top">
                      <GrowButton link type="primary" @click="goDesign(row)">
                        <GrowIconify icon="ant-design:bar-chart-outlined" :size="16" />
                      </GrowButton>
                    </GrowTooltip>
    ...
    <div class="report-asset-manage__pager">
      <GrowPagination
    ...
    <GrowDialog
      v-model="formVisible"
      :title="formMode === 'create' ? '新增报表资产' : '编辑基础信息'"
      width="520px"
      append-to-body
      destroy-on-close
    >
    ...
    <GrowDialog
      ...
      destroy-on-close
    >
      <p class="report-asset-manage__hint">
    ...
    <GrowDrawer
      ...
      destroy-on-close
    >
      <div class="report-asset-manage__version-head">
        <div class="report-asset-manage__version-head-main">
          <span>{{ versionAsset?.name }}</span>
          <span v-if="versionAsset?.code" class="report-asset-manage__muted">
            {{ versionAsset.code }}
          </span>
        </div>
        <span class="report-asset-manage__muted">
          当前版本：{{ versionCurrent || '-' }}
        </span>
      </div>
      <p v-if="versionLoading" class="report-asset-manage__hint">加载中…</p>
      <div v-else-if="!versionRows.length" class="report-asset-manage__version-empty">
        暂无发布版本
      </div>
      <GrowTimeline v-else class="report-asset-manage__version-timeline">
        <GrowTimelineItem
        ...
        >
          <div
            class="report-asset-manage__version-card"
            :class="{ 'is-current': row.isCurrent }"
          >
            <div class="report-asset-manage__version-card-top">
              <div class="report-asset-manage__version-card-title">
                <span class="report-asset-manage__version-no">{{ row.version }}</span>
            ...
            <div class="report-asset-manage__version-meta">
              <div class="report-asset-manage__version-meta-item">
                <span class="report-asset-manage__version-meta-label">发布时间</span>
                <span>{{ formatTime(row.publishedAt) }}</span>
              </div>
              <div class="report-asset-manage__version-meta-item">
                <span class="report-asset-manage__version-meta-label">发布人</span>
                <span>{{ row.publishedBy || '-' }}</span>
              </div>
            </div>
            <div class="report-asset-manage__version-remark-block">
              <span class="report-asset-manage__version-meta-label">发布备注</span>
              <p class="report-asset-manage__version-remark">
                {{ row.remark || '无备注' }}
              </p>
            </div>
          </div>
...
import { useReportAssetManage } from './use/useReportAssetManage'

defineOptions({
  name: 'ReportAssetManagePage',
})
...
.report-asset-manage__version-head {
  display: flex;
...
.report-asset-manage__version-head-main {
  display: flex;
  flex-direction: column;
  gap: 4px;
  min-width: 0;
...
.report-asset-manage__version-empty {
  padding: 40px 0;
  text-align: center;
  color: var(--text-color-secondary);
  font-size: 13px;
...
.report-asset-manage__version-timeline {
  padding-left: 4px;
}
...
.report-asset-manage__version-card-top {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 8px;
...
.report-asset-manage__version-card-title {
  display: flex;
  align-items: center;
  gap: 8px;
  min-width: 0;
...
.report-asset-manage__version-no {
  font-size: 15px;
  font-weight: 600;
  color: var(--text-color);
}

.report-asset-manage__version-meta {
  display: grid;
...
.report-asset-manage__version-meta-item {
  display: flex;
  flex-direction: column;
  gap: 2px;
  min-width: 0;
...
.report-asset-manage__version-meta-label {
  font-size: 12px;
  color: var(--text-color-secondary);
}

.report-asset-manage__version-remark-block {
  display: flex;
  flex-direction: column;
  gap: 4px;
  margin-top: 10px;
...
.report-asset-manage__version-remark {
  margin: 0;
  font-size: 13px;
  line-height: 1.5;
  color: var(--text-color);
...
.report-asset-manage__muted {
  color: var(--text-color-secondary);
  font-weight: 400;
  font-size: 13px;
}

.report-asset-manage__actions {
  display: inline-flex;
  align-items: center;
  gap: 2px;
  flex-wrap: nowrap;
